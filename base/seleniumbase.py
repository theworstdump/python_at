...
class SeleniumBase:
    def __init__(self, driver):
        self.driver = driver
        self.__wait = WebDriverWait(driver, 15, ignored_exceptions=StaleElementReferenceException)

    def __get_selenium_by(self, find_by: str) -> dict: # "__" - private method
    ...
    def get_text_from_webelements(self, elements: List[WebElement]) -> List[str]:
        ...
        #     element_list.append(element.text) # строчка ниже является более компактной альтернативой цикла for
        return [element.text for element in elements] # about list comprehension https://webdevblog.ru/kogda-ispolzovat-list-comprehension-v-python/
